...
## Калибровка 03.09: честные выборки и потолок гибрида

Собраны реальные выборки (scripts/collect_heldout.py, 60 дней pravo.gov,
~23k документов; data/heldout_samples.json):
- Выборка 2 (25 held-out «потерянных Stage A»).
- Выборка 3 пересобрана по консультации claude (/tmp/claude_threshold_advice.md):
  15 «твёрдых» негативов с реальным topic+marker совпадением (эффективный n был 2!),
  2 «лёгких», спорный Минздрав Марий Эл — отдельной uncertain-меткой, не в метрике.
- Метрика для all-negative выборки 3 — specificity tn/(tn+fp), не precision
  (precision для неё по построению n/a|0.00) — фикс в calibrate_hybrid.py.

**Диагноз по FP (4 шт. на честной выборке 3):** якоря вида «меры социальной
поддержки ветеранов боевых действий» давали BM25 до 7.5 на любом «О мерах
социальной поддержки <чужая группа>» — сигнал только в отличительной части якоря.

**Попытка 1 — стоп-слова BM25** (_BM25_STOP_STEMS): specificity 0.76→0.88 ✅,
но recall 0.64→0.36 ❌ — истинные срабатывания тоже держались на общих словах.

**Попытка 2 — разделение якорей** (полные для косинуса, отличительные для BM25,
claude): grid search по порогам (/tmp/grid2.out): recall 0.24–0.28 при
specificity 0.88–0.94 — recall перестал зависеть от t_bm25 (BM25 перестал
подтверждать истинные), косинус rubert-tiny2 на коротких заголовках до порога
не дотягивается.

**Вердикт:** гибрид BM25(rubert-tiny2-cosine) на этом признаковом пространстве
не проходит приёмку (recall ≥0.60 И specificity ≥0.85 одновременно недостижимы
ни в одной точке сетки). Альтернативы для следующей итерации:
1) сильный эмбеддер (Qwen3-Embedding-0.6B из auto) вместо rubert-tiny2;
2) LLM-ассист на границе решений (низкий confidence);
3) расширение якорей-эмбеддингов парами «заголовок→категория» из размеченной БД.
До выбора альтернативы Stage B в проде работает на стартовых порогах
(консервативно: specificity 1.00, recall 0 — т.е. фактически отключён по
приёмке, ложных сигналов не даёт).
